...
public sealed class AtualizarPatioHandler
{
    ...
    public async Task<bool> ExecuteAsync(int id, AtualizarPatioRequest req, CancellationToken ct = default)
    {
        var e = await _repo.GetByIdAsync(id, ct);
        if (e is null) return false;
        e.Atualizar(req.NomePatio, req.TamanhoPatio, req.Andar, req.FilialId);
